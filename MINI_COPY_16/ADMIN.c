    ...
    switch (choice) {
        ...
        case 7:
            ...
            if(choice2[0] == 1 ){
                ...
                strcpy(student_update.name,k);
                write(upd,&student_update,sizeof(struct student));
            }
            else if(choice2[0] == 2 ){
                char new_email[50];
                send(client_socket,"Enter updated email",100,0);
                recv(client_socket,new_email,sizeof(new_email),0);
                strcpy(student_update.email,new_email);
                write(upd,&student_update,sizeof(struct student));
            }
            else if(choice2[0] == 3){
                char new_Address[50];
                ...
                strcpy(student_update.address,new_Address);
                write(upd,&student_update,sizeof(struct student));
            }
            close(upd);
            char repl1_[20];
            ...
            break;
        case 8:
            int upd1 = open("faculty.txt",O_RDWR);
            char a2[50];
            send(client_socket,"Enter faculty id to update",30,0);
            recv(client_socket,a2,sizeof(a2),0);
            char targetfacultyID[100];
            strcpy(targetfacultyID,a2);
            struct Faculty faculty_update;

            int numID1;
            sscanf(a2,"iiitb@%d", &numID1);
            int offset2 = (numID1-1)*sizeof(struct Faculty);
            lseek(upd1,offset2,SEEK_SET);
            read(upd1,&faculty_update,sizeof(struct Faculty));
            send(client_socket,"Choose from the following which information you want to update\n 1) Name\n 2)Email \n 3) Address\n",1000,0);
            int choice3[2];
            recv(client_socket,choice3,sizeof(choice3),0);
            lseek(upd1,-(sizeof(struct Faculty)),SEEK_CUR);
            if(choice3[0] == 1 ){
                char k1[50];
                send(client_socket,"Enter the updated name",100,0);
                recv(client_socket,k1,sizeof(k1),0);
                strcpy(faculty_update.name,k1);
                write(upd1,&faculty_update,sizeof(struct Faculty));
            }
            else if(choice3[0] == 2 ){
                char new_email1[50];
                send(client_socket,"Enter updated email",100,0);
                recv(client_socket,new_email1,sizeof(new_email1),0);
                strcpy(faculty_update.email,new_email1);
                write(upd1,&faculty_update,sizeof(struct Faculty));
            }
            else if(choice3[0] == 3){
                char new_Address1[50];
                send(client_socket,"Enter updated Address",100,0);
                recv(client_socket,new_Address1,sizeof(new_Address1),0);
                strcpy(faculty_update.address,new_Address1);
                write(upd1,&faculty_update,sizeof(struct Faculty));
            }
            close(upd1);
            char repl11_[20];
            send(client_socket,"Done",5,0);
            recv(client_socket,repl11_,sizeof(repl11_),0);
            break;
        case 9:
            *admin_exit = 1;
            break;
...
void Admin(int choice,int client_socket,int *admin_exit1){
        ...
        case 8:
            char toupdate1[50];
            recv(client_socket,toupdate1,sizeof(toupdate1),0);
            char inp1[50];
            printf("%s\n",toupdate1);
            scanf("%s",inp1);
            send(client_socket,inp1,sizeof(inp1),0);

            char DATA11[1000];
            recv(client_socket,DATA11,sizeof(DATA11),0);
            printf("%s\n",DATA11);
            int choice11[1];
            scanf("%d",&choice11[0]);
            send(client_socket,choice11,sizeof(choice11),0);

            char DATA_[100];
            recv(client_socket,DATA_,sizeof(DATA_),0);
            printf("%s\n",DATA_);
       
            char DATA22[100];
            scanf("%s",DATA22);
            send(client_socket,DATA22,sizeof(DATA22),0);

            char ackn_[10];
            recv(client_socket,ackn_,sizeof(ackn_),0);
            send(client_socket,"OK",5,0);
            break;
        case 9:
            *admin_exit1 = 1;
